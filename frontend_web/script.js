...
function setVersion(version) {
  ...
  $('versionDefault').classList.toggle('active', version === 'default');
  $('versionNoYt').classList.toggle('active', version === 'no_yt');
}
...
async function checkHealth() {
  ...
  try {
    ...
    if (d.models_ready) {
      ...
      const vLabel = d.model_version_name || d.model_version || '';
      $('statusText').textContent = `ONLINE · ${d.folds_loaded.length} FOLDS · ${vLabel.toUpperCase()}`;
    } else {
...
async function predict() {
  ...
  const base = $('apiUrl').value.replace(/\/$/, '');
  const endpointPath = currentMode === 'ensemble' ? '/predict/ensemble' : '/predict';
  const endpoint = `${endpointPath}?model_version=${currentVersion}`;
...
function showResult(d) {
  ...
  $('detVersion').textContent = d.model_version_name || VERSION_LABELS[d.model_version] || d.model_version || '—';
...
function validateLink() {
  ...
  if (url && url.value.trim() === '') {
    showError('PLEASE ENTER A VALID EXTRANET SOURCE URL.');
    return;
  }
  showError('REMOTE STREAM INGESTION OFFLINE.');
}
